...
/**
 * Library UI Renderer (Final Polish: Contrast, Widths & Icons)
 */
function refreshLibraryUI() {
    ...
    if (currentShotData && currentShotData.profile && !librarySearch[DB_KEYS.PROFILES]) {
        const target = cleanStr(currentShotData.profile);
        profiles.sort((a, b) => {
    ...
    if (currentProfileData && !librarySearch[DB_KEYS.SHOTS]) {
        const target = cleanStr(currentProfileName);
        shots.sort((a, b) => {
    ...
    // Chevrons (Stroke widths handled by CSS, but good to have clean SVG)
    ...
    const createSection = (title, items, type) => {
        ...
        // --- DYNAMIC COLUMN WIDTHS (Optimized) ---
        // Shot table: Thin name, wider profile
                    ...
                    <div class="lib-toolbar-left">
                        <div class="lib-search-wrapper">
                            ...
                            <input type="text" id="search-${type}" class="lib-search" 
                                   placeholder="Search ${title}..." value="${librarySearch[type]}" 
                    ...
                    <table class="lib-table">
                        ...
                        <tbody>
                            ${items.length === 0 ? `<tr><td colspan="4" class="empty-msg" style="padding:15px; text-align:center; color:#999;">No results found</td></tr>` : items.map(item => {
                                ...
                                const cleanProfileRef = (currentShotData && currentShotData.profile) ? cleanStr(currentShotData.profile) : "";
                                const cleanCurrentProfile = currentProfileName ? cleanStr(currentProfileName) : "";
    ...
    if (currentShotData && currentProfileData) {
        const shotRef = cleanStr(currentShotData.profile);
        const activeLabel = cleanStr(currentProfileName);
        ...
        <div class="library-status-bar">
            <div class="status-bar-group">
                ...
                <div class="${shotClass}" onclick="window.toggleStickyPanel()">
                    ...
                    <span class="status-value">${currentShotName.replace(/\.json$/i, '')}</span>
            ...
            <div class="status-bar-group">
                <div class="${profileClass}" onclick="window.toggleStickyPanel()" title="${mismatchTitle}">
                    <div class="badge-left">${badgeChevron}</div>
                    <span class="status-value">${profileClass.includes('mismatch') ? '⚠ ' : ''}${currentProfileName.replace(/\.json$/i, '')}</span>
